...
export default function AdminWeightsScreen() {
  ...
  const activePreset = useMemo(
    () =>
      weightPresetCatalog.find(
        (preset) => preset.key === adminState?.weightPreset,
      ),
  ...
  const handlePresetChange = (presetKey: AdminState["weightPreset"]) => {
    ...
    dispatch({
      type: "update",
      payload: { ...adminState, weightPreset: presetKey },
    });
  ...
  const handleVerificationToggle = () => {
    ...
    const optimistic: AdminState = {
      ...adminState,
      verificationPolicy:
        adminState.verificationPolicy === "strict" ? "relaxed" : "strict",
    };
    ...
    <ScrollView contentContainerStyle={styles.container}>
      <ThemedView style={[styles.hero, { borderColor: theme.icon }]}>
        ...
        <View style={styles.badgeRow}>
          <Badge
            label={`現在: ${activePreset?.title ?? "読込中"}`}
      ...
      {isLoading ? (
        <ThemedView
          style={[
            styles.card,
            { borderColor: theme.icon, alignItems: "center" },
          ]}
        >
          <ActivityIndicator />
      ...
      {adminState ? (
        <ThemedView style={[styles.card, { borderColor: theme.icon, gap: 12 }]}>
            ...
            {weightPresetCatalog.map((preset) => (
              <Pressable
                ...
                style={({ pressed }) => [
                  ...
                  {
                    borderColor:
                      preset.key === adminState.weightPreset
                        ? theme.tint
                        : theme.icon,
                    backgroundColor:
                      preset.key === adminState.weightPreset
          ...
          <ThemedText style={styles.helperText}>
            ・厳格モード: 大学メール +
            学籍証明を必須にし、未審査ユーザーを非表示。
            {"\n"}・柔軟モード:
            大学メール認証をベースにしつつ、新規ユーザーの表示を拡大。
          </ThemedText>
        </ThemedView>
      ) : null}
...
function WeightChip({
  ...
  return (
    <View
      style={[
        styles.weightChip,
        { backgroundColor: `${themeColor}12`, borderColor: `${themeColor}50` },
      ]}
    >
