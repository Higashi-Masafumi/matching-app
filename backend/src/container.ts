...
export const createContainer = () => {
  ...
  const universityRepository = new UniversityRepositoryDrizzle(databaseClient.db);
  const profileRepository = new ProfileRepositoryDrizzle(databaseClient.db);
  const configurationRepository = new ConfigurationRepositoryDrizzle(databaseClient.db);
  ...
  return {
    ...
    getCatalogConfigurationUseCase: new GetCatalogConfigurationUseCase(configurationRepository),
  };
};
