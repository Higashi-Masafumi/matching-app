  ...
  generateCandidates(current: Profile, others: Profile[]): MatchCandidate[] {
    return others
      ...
      .map((profile) => {
        ...
        const sharedMajors = profile.majors.filter((major) =>
          current.majors.includes(major),
        );
        ...
        const interestScore =
          sharedInterests.length / Math.max(profile.interests.length, 1);
        const majorScore =
          sharedMajors.length / Math.max(profile.majors.length, 1);
        const languageScore =
          sharedLanguages.length / Math.max(profile.languages.length, 1);
