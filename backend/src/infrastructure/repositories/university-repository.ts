  ...
  async list(params: ListUniversitiesParams): Promise<University[]> {
    ...
    const filtered = rows
      .filter((row) => {
        ...
        if (params.country) {
          matches = matches && row.country.toLowerCase() === params.country.toLowerCase();
        }
        ...
        if (params.program) {
          ...
          matches =
            matches &&
            programs.some((program) => program.toLowerCase().includes(params.program!.toLowerCase()));
        }

        if (params.search) {
          const haystack = `${row.name} ${row.city} ${row.region}`.toLowerCase();
    ...
    return filtered.map(
      (row) =>
        ({
          ...
          verificationLevel: row.verificationLevel as University['verificationLevel'],
