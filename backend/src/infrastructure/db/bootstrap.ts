...
const intentOptionSeeds = [
  {
    id: "same",
    label: "同じ大学でマッチ",
    description: "学内コミュニティを固めたい",
    radiusKm: 0,
  },
  {
    id: "nearby",
    label: "近隣大学と繋がる",
    description: "同じエリアでイベントをしたい",
    radiusKm: 30,
  },
  {
    id: "open",
    label: "全国どこでも",
    description: "進学・交換留学の相談をしたい",
    radiusKm: null,
  },
];
...
export const bootstrapDatabase = (connection: BetterSqlite3Instance) => {
  ...
  seedTable(connection, VERIFICATION_FLAG_TABLE, () => {
    ...
    const insertMany = connection.transaction(
      (rows: typeof verificationFlagSeeds) => {
        for (const row of rows) {
